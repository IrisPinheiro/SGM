<?php
    ...
    if(!empty($_GET['id'])){
        ...
        $sqlSelect = "SELECT * FROM formulario.relatorio WHERE id=$id";
        ...
        if($result->num_rows >0){
            ...
            while($user_data = mysqli_fetch_assoc($result)){
                ...
                $status_os = $user_data['status_os'];
                $data_conclusao = $user_data['data_conclusão'];
            }
        }else{
...
<body>
    <div class="container">
        ...
        <form action="saveEdit_os.php" method="POST">
    
            <div class="box" id="bx">

            <br>
            <label >Nº_OS</label>
            <input type="text" id="num" name="num" required class="input" autocomplete="off" placeholder="000/00" value="<?php echo $num?>">

            <label >Data de emissão</label>
            <input type="date" id="data" name="data_emissao" required value="<?php echo $data_emissao?>">

            <label >Data de conclusão</label>
            <input type="date" id="data" name="data_conclusao" required value="<?php echo $data_conclusao?>" ><br><br><br>
            ...
            <input type="text" id="id_cliente" name="id_cliente" class="input" autocomplete="off" value="<?php echo $id_cliente?>"><br><br>
            ...
            <input type="text" id="responsavel" name="responsavel" class="input" autocomplete="off" value="<?php echo $responsavel?>"><br><br>
            <br><br>
            <div class="status">
                <input type="radio" name=" status_os" id=" exe" value="Em Execução" <?php echo ($status_os == 'Em Execução') ? 'checked' : ''?>>
                <label class="form-check-label" for=" exe">Executar</label>

                <input type="radio" name=" status_os" id=" feito" value="Finalizada"  <?php echo ($status_os == 'Finalizada') ? 'checked' : ''?>>
                <label class="form-check-label" for=" feito">Finalizar</label>
            </div>
            <br><br>
        ...
        <button id="update" name="update" type="submit">Atualizar Status</button>
        <br><br><br>
        </form>
